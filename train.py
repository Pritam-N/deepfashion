...
def generate_label_plain(inputs):
    ...
    pred_batch = []
    for input in inputs:
        input = input.view(1, NC, 1008, 672)
        pred = np.squeeze(input.data.max(1)[1].cpu().numpy(), axis=0)
        pred_batch.append(pred)

    pred_batch = np.array(pred_batch)
    pred_batch = torch.from_numpy(pred_batch)
    label_batch = pred_batch.view(size[0], 1, 1008, 672)
...
for epoch in range(start_epoch, opt.niter + opt.niter_decay + 1):
    ...
    for i, data in enumerate(dataset, start=epoch_iter):
        ...
        backward_t2 = time.time()
        #
        # # update discriminator weights
        loss_D.backward()
        backward_t3 = time.time()
        # batchsize 64
        if (i+1) % 64 == 0:
            model.module.optimizer_D.step()
            model.module.optimizer_D.zero_grad()
        backward_t4 = time.time()
        ...
        forward = forward_end - forward_strt
        backward_t10 = (backward_t1 - backward_t0)*1000
        backward_t21 = (backward_t2 - backward_t1)*1000
        backward_t32 = (backward_t3 - backward_t2)*1000
        backward_t43 = (backward_t4 - backward_t3)*1000
        total_lapse = (forward+backward_t10+backward_t21 +
                       backward_t32+backward_t43)*1000

        log_path = os.path.join(opt.checkpoints_dir, opt.name, 'time_log.txt')

        with open(log_path, 'a+') as filewriter:
            filewriter.write(
                '[step:{}] [forward pass time:{} milliseconds]\n'.format(step, forward))
            filewriter.write(
                '[step:{}] [backward pass loss G:{} milliseconds]\n'.format(step, backward_t10))
            filewriter.write('[step:{}] [backward pass optimizer G:{} milliseconds]\n'.format(
                step, backward_t21))
            filewriter.write(
                '[step:{}] [backward pass loss D:{} milliseconds]\n'.format(step, backward_t32))
            filewriter.write('[step:{}] [backward pass optimizer D:{} milliseconds]\n'.format(
                step, backward_t43))
            filewriter.write(
                '[step:{}] [total time lapse:{} milliseconds]\n'.format(step, total_lapse))
            filewriter.write('\n')
