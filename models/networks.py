...
class CNN(nn.Module):
    def __init__(self, num_output, input_nc=5, ngf=8, n_layers=5, norm_layer=nn.InstanceNorm2d, use_dropout=False):
        ...
        model = [downconv, norm_layer(ngf), nn.ReLU(True)]
        ...
        # model += [nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1),
        #           norm_layer(64), nn.ReLU(True)]
        model += [nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1),
        ...
        self.model = nn.Sequential(*model)
        #self.fc1 = nn.Linear(2048, 128) # 256x192 IMAGES
        #self.fc1 = nn.Linear(2048, 128) # 512x336, 512x352 IMAGES
        #self.fc1 = nn.Linear(13824, 128) # 1248x832 IMAGES
        self.fc1 = nn.Linear(8960, 128) # 1008x672 IMAGES
...
class STNNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.tps = TPSGridGen(1008,672, target_control_points)
    ...
    def forward(self, x, reference, mask):
        ...
        grid = source_coordinate.view(batch_size, 1008, 672, 2)
